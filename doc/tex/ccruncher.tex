...
Moreover gaussian copula is too optimistic on diversiﬁcation beneﬁts, while the Gumbel 
...
Algorithm to simulate a t-Sudent copula with $\nu$ degrees of freedom satisfying 
correlation matrix $\Sigma$ \cite{copu:pricing}:
...
$\vec{Z} \sim \frac{\sqrt{d}}{\sqrt{\chi^2(\nu)}} \cdot N(\vec{0}, \Sigma')$, doing:
\begin{displaymath}

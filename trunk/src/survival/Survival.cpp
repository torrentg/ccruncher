...
double ccruncher::Survival::inverse(const int irating, double val) const
{
  ...
  int k = (int)(ceil(val*ISURVFNUMBINS)); //ceil because survival functions are decreasing
  int i0 = (int) idata[irating][k];
  int i1 = i0 + 1;
  ...
  //TODO: check that val is in interval [ddata[irating][i0], ddata[irating][i1]]

  double x0 = ddata[irating][i0];
  double y0 = (double)(i0);
  double x1 = ddata[irating][i1];
  double y1 = (double)(i1);

...
Sea $Z$ la variable aleatoria que modela el valor de la cartera en el
tiempo $T$. Sea $F$ la funci\'on de distribuci\'on (cdf) de $Z$.
En la figura \ref{creditvar} se muestran los conceptos expuestos en
este apartado.
...
Responde a la pregunta \emph{Cual es la p\'erdida m\'inima incurrida en el
$X$\% de los peores casos?}. Se recomienda la lectura del libro
\emph{Value at Risk} \cite{var:jorion}.
\begin{displaymath}
VAR_{\alpha}(Z) = \textrm{inf}\{z | F(Z) \geq 1-\alpha \}
\end{displaymath}
...
Responde a la pregunta \emph{Cual es la p\'erdida media incurrida en el $X$\% de
los peores casos?}. Se recomienda la lectura de los art\'iculos \cite{var:varbad}
y \cite{var:eshortfall}.
\begin{displaymath}
TCE_{\alpha}(Z) = E(Z | Z < VAR_{\alpha}(Z))
\end{displaymath}

\paragraph{Definici\'on.} El \emph{Valor Esperado}\index{Valor esperado} es:
\begin{displaymath}
\textrm{Expected Value} = E(Z)
\end{displaymath}

\paragraph{Definici\'on.} La \emph{P\'erdida Esperada}\index{P\'erdida esperada} es:
\begin{displaymath}
\textrm{Expected Loss} = K - E(Z)
\end{displaymath}
donde $K$ es el valor de la cartera en el caso en que ning\'un creditor haga fallido.

\paragraph{Definici\'on.} Definimos el \emph{Capital Econ\'omico}\index{Capital econ\'omico}
al nivel de confianza $\alpha$ como:
\begin{displaymath}

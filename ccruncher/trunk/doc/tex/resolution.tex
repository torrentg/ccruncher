...
Consideramos que el valor del activo $j$ del cliente $i$ en tiempo $t$ es:
\begin{displaymath}
\begin{array}{rl}
X_{i,j}(t) = \sum_{t_k=t_0}^{t} & \textrm{cashflow}(t_k) \cdot \Upsilon(t_k,t_0, S_{t_0}) \cdot [1-I_{Default}(t_k)] \cr
+ &\textrm{recovery}(t_k) \cdot \Upsilon(t_k,t_0, S_{t_0}) \cdot [I_{Default}(t_k)] \cr
- &\textrm{exposure}(t_k) \cdot \Upsilon(t_k,t_0, S_{t_0}) \cdot [I_{Default}(t_k)]  
\end{array}
\end{displaymath}
...
I_{Default}(t) = \left\{
\begin{array}{ll}
1 & \textrm{ if client has defaulted at } t \cr
0 & \textrm{ otherwise}
\end{array}
\right.
\end{displaymath}
...
Deseamos obtener la distribuci\'on del valor de la cartera en tiempo $T$, $Z(T)$.
\begin{displaymath}

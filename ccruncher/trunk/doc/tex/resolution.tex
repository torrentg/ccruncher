...
\paragraph{Expected value.} F\'ormula extraida de \cite{stats:schaum}.
\begin{displaymath}
\mu_Z = \widehat{\mu_Z} \pm \phi^{-1}\left(\frac{1-\alpha}{2}\right) \cdot \frac{\widehat{\sigma_Z}}{\sqrt{N}}
\end{displaymath}

\paragraph{Desviaci\'on est\'andar.} F\'ormula extraida de \cite{stats:schaum}.
\begin{displaymath}
\sigma_Z = \widehat{\sigma_Z} \pm \phi^{-1}\left(\frac{1-\alpha}{2}\right) \cdot \frac{\widehat{\sigma_Z}}{\sqrt{2N}}
\end{displaymath}

\paragraph{Percentil (VAR).} F\'ormula extraida de \cite{???:???}.
\begin{displaymath}
??? searching - searching ??? - Confidence intervals - quantile - typical error - order statistics
\end{displaymath}

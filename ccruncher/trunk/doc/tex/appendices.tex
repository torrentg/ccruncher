...
\begin{displaymath}
VAR_{\alpha}(X) = \textrm{inf}\{x | F(X) \geq 1-\alpha \} \approx x_{k:n}
\quad \textrm{where} \quad \frac{k}{n} \leq 1-\alpha < \frac{k+1}{n}
\end{displaymath}
donde $F$ es la funci\'on de distribuci\'on de $X$ y $x_{k:n}$ es el $k$-\'esimo
elemento de la serie ordenada de forma creciente.

\paragraph{Observaci\'on.} Los estimadores definidos son no sesgados.

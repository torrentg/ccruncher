...
%
%
% appendices.tex - TeX documentation file - $Rev$
% --------------------------------------------------------------------------
%
...
\paragraph{Proposici\'on.} Estimamos la media de $X$ usando el siguiente
...
\paragraph{Proposici\'on.} Estimamos el VAR\index{Value At Risk} de $X$ al nivel de
confianza $\alpha$ usando el siguiente estimador:
\begin{displaymath}
...
\paragraph{Proposici\'on.} Estimamos el TCE (Tail Conditional Expectation o Expected
Shortfall) \index{Tail Conditional Expectation}\index{Expected Shortfall}
...
\index{Error est\'andar}
\index{Tail Conditional Expectation}
...
\paragraph{Paso 2.} Definimos la variable aleatoria $Y = \{X|X > VAR_{\alpha}(X)$\}.
...
\end{displaymath}
Para calcular la inversa de una matriz se calcula la descomposici\'on
$LU$\index{Algoritmo LU} de dicha matriz y se resuelve el sistema
$L \cdot U \cdot P^{-1} = Id$ elemento a elemento. El m\'etodo $LU$ y la inversa de
una matriz usando la descomposici\'on $LU$ se encuentra explicada con detalle en el
libro \emph{Numerical Recipes in C}\footnote{http://www.nr.com}.
...
\index{C\'opula gaussiana} Sea $\Sigma_1$ la matriz de correlaci\'on a cumplir
...
Cholesky\index{Algoritmo de Cholesky} para obtener la matriz triangular
...
\footnote{El algoritmo de simulaci\'on de una variable aleatoia Normal(0,1) se
encuentra explicado en el apartado \ref{sec:normal}}:
\begin{displaymath}
\vec{Y} =
\left(
\begin{array}{c}

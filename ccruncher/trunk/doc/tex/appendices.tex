...
\paragraph{Definici\'on.} Estimamos la media de $X$ usando el siguiente
...
\begin{displaymath}
VAR_{\alpha}(X) = \textrm{inf}\{x | F(X) \geq 1-\alpha \} \approx \widehat{VAR_{\alpha}(X)} = x_{k:n}
\end{displaymath}
donde,
\begin{itemize}
\item $F$ es la funci\'on de distribuci\'on de $X$
\item $k$ cumple $\frac{k}{n} \leq 1-\alpha < \frac{k+1}{n}$
...
\frac{\sum_{i=1}^{n} x_i \cdot \textrm{1}_{\{x_i \le  x_{k:n}\}}}{\sum_{i=1}^{n} \textrm{1}_{\{x_i \le  x_{k:n}\}}}
\end{displaymath}
donde,
\begin{itemize}
\item $F$ es la funci\'on de distribuci\'on de $X$
...
estad\'istico de orden. Sea $N$ el tama\~no muestral y $x$ la probabilidad
del quantil.
...
de una variable aleatoria condicionada a que sea inferior a un cierto quantil.
...
\paragraph{Paso 2.} Definimos la variable aleatoria $Y = \{X|X \le VAR_{\alpha}(X)$\}.
Generamos un conjunto de observaciones de $Y$ seleccionamos las observaciones de $X$
que son inferiores a $VAR_{\alpha}(X)$:
\begin{displaymath}
...
\begin{eqnarray}
\widehat{TEC_{\alpha}(X)} & = & \widehat{\mu_Y} \nonumber \\
\textrm{stderr}(TEC_\alpha) & = & \frac{\widehat{\sigma_Y}}{\sqrt{M}} \nonumber
\end{eqnarray}
...
Cholesky\index{Algoritmo de Cholesky} para obtener la matrix triangular
